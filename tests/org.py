...
class TestOrgs(unittest.TestCase):
	...
	def test_dogear(self):
		...
		self.assertEqual(u"Nobuo Uematsu", dogear['staff'][0]['name']['en'])
		self.assertEqual(u"Miyu", dogear['staff'][1]['name']['en'])
		...
		self.assertEqual(u"Kalaycilar", dogear['releases'][2]['name']['en'])
	...
	def test_vagrancy(self):
		...
		self.assertEqual(u"Akiko Shikata", vagrancy['staff'][0]['name']['en'])
		self.assertEqual(u"志方あきこ", vagrancy['staff'][0]['name']['ja'])
