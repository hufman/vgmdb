...
class TestSearchList(unittest.TestCase):
	...
	def test_search(self):
		...
		self.assertEqual(u"album/71068", search['results']['albums'][0]['link'])
		self.assertEqual(u"Vagrant Story Original Soundtrack", search['results']['albums'][1]['titles']['en'])
		self.assertEqual(u"MICA-0368~9", search['results']['albums'][2]['catalog'])
		self.assertEqual(u"2000", search['results']['albums'][1]['release_date'])
		self.assertEqual(u"2003-12-28", search['results']['albums'][4]['release_date'])
