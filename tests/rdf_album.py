...
class TestAlbumsRDF(TestRDF):
	...
	def run_ff8_tests(self, graph):
		test_count_results = {
			...
			"select ?person where { ?person foaf:made <@base#subject> . }" : 4,
			...
			"select ?person where { ?person foaf:made <@base#lyrics> . }" : 3,
		...
		test_first_result = {
			...
			"select ?rating where { <@base#subject> schema:aggregateRating ?agg . ?agg schema:ratingValue ?rating . }" : decimal.Decimal("4.44"),
			"select ?rating where { <@base#subject> schema:aggregateRating ?agg . ?agg schema:ratingCount ?rating . }" : 57,
